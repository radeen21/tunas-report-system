...
type ImportAcademicRow = {
  ...
  "Tugas 1"?: string | number;
  "Tugas 2"?: string | number;
  "Tugas 3"?: string | number;
  "Tugas 4"?: string | number;
  "Tugas 5"?: string | number;

  UTS?: string | number;
  UAS?: string | number;

  "Proses KBM"?: string | number;
...
async function uploadAcademicReportFile(
  file: File,
  teacherId: string
) {
  const safeFileName =
...
export default function TeacherAcademicReportsPage() {
  ...
  async function fetchData() {
    ...
    try {
      ...
      const enrichedReports: EnrichedReport[] =
        reportsData.map(
          (report) => {
            const student =
              report.student_id
      ...
      if (
        ...
        setErrorMessage(
          "Gagal mengambil data laporan akademik."
        );
      }
  ...
  useEffect(() => {
    ...
    const channel =
      supabase
        .channel(
          "teacher-academic-reports-realtime"
        )
        .on(
          "postgres_changes",
          {
  ...
  /*
   * Status report yang sedang dibuka.
   * Dibutuhkan supaya Pending yang diedit
   * tidak otomatis berubah jadi Draft.
   */
  const editingReportStatus =
    useMemo(() => {
      if (!form.id) {
        return "draft";
      }
  ...
  const studentOptionsForForm =
    useMemo(() => {
      const studentIdsFromRelation =
        new Set(
          studentTeachers
            .map(
              (relation) =>
    ...
    useMemo(() => {
      if (
        ...
        return subjects.filter(
          (subject) =>
            subjectIdsFromRelation.has(
              subject.id
            )
        );
      }

      const subjectIdsForSelectedStudent =
        new Set(
          studentTeachers
            .filter(
              (relation) =>
      ...
      return subjects.filter(
        (subject) =>
          subjectIdsForSelectedStudent.has(
            subject.id
          )
      );
    }, [
  ...
  function openEditModal(
    ...
    if (
      ...
      alert(
        "Laporan yang sudah approved tidak bisa diedit oleh guru."
      );
    ...
    setForm({
      ...
      uh_1:
        report.uh_1?.toString() ||
        report.uh_score?.toString() ||
        "",

      uh_2:
        report.uh_2?.toString() ||
        "",

      uh_3:
        report.uh_3?.toString() ||
        "",

      uh_4:
        report.uh_4?.toString() ||
        "",

      task_1:
        report.task_1?.toString() ||
        report.task_score?.toString() ||
        "",

      task_2:
        report.task_2?.toString() ||
        "",

      task_3:
        report.task_3?.toString() ||
        "",

      task_4:
        report.task_4?.toString() ||
        "",

      task_5:
        report.task_5?.toString() ||
        "",

      mid_score:
        report.mid_score?.toString() ||
        report.uts_score?.toString() ||
        "",

      final_exam_score:
        report.final_exam_score?.toString() ||
        report.uas_score?.toString() ||
        "",

      process_score:
        report.process_score?.toString() ||
        "",
  ...
  function validateForm() {
    ...
    if (
      ...
      setErrorMessage(
        "Siswa ini belum terhubung dengan guru login."
      );
    ...
    if (
      !form.report_period.trim()
    ) {
      setErrorMessage(
        "Isi periode laporan terlebih dahulu."
      );
    ...
    if (
      ...
      setErrorMessage(
        "File harus PDF, Word, Excel, JPG, JPEG, atau PNG."
      );
  ...
  async function buildPayload(
    ...
    if (
      ...
      reportFileUrl =
        await uploadAcademicReportFile(
          selectedFile,
          teacher.id
        );
    }
    ...
    return {
      ...
      final_exam_score:
        calculated.finalExamScore,

      average_uh:
        calculated.averageUh,

      average_task:
        calculated.averageTask,

      process_score:
        calculated.processScore,
  ...
  async function saveReport(
    ...
    try {
      ...
      if (form.id) {
        ...
        /*
         * Jangan mengganti
         * submitted_at lama saat
         * Pending hanya diedit.
         */
        if (
          approvalStatus ===
            "pending" &&
        ...
        if (
          ...
          if (
            ...
            throw new Error(
              "Laporan siswa, mapel, dan periode ini sudah approved dan tidak bisa diubah oleh guru."
            );
          }

          const duplicateEffectiveStatus:
            | "draft"
            | "pending" =
            duplicateStatus ===
            "pending"
  ...
  async function quickSubmitReport(
    ...
    if (
      ...
      alert(
        approvalStatus ===
          "pending"
          ? "Laporan ini sudah menunggu approval Kepala Sekolah."
          : "Laporan ini sudah approved dan tidak bisa disubmit ulang."
      );
    ...
    if (error) {
      alert(
        `Gagal submit laporan: ${error.message}`
      );
  ...
  function handleDownloadTemplateExcel() {
    ...
    XLSX.writeFile(
      workbook,
      `template-nilai-akademik-${formatExportDateName()}.xlsx`
    );
  }
  ...
  async function handleImportExcel(
    ...
    try {
      ...
      const rows =
        XLSX.utils.sheet_to_json<ImportAcademicRow>(
          sheet,
          {
            defval: "",
            raw: false,
      ...
      for (
        ...
        if (!student) {
          ...
          errors.push(
            `Baris ${rowNumber}: siswa tidak ditemukan.`
          );
        ...
        if (!subject) {
          ...
          errors.push(
            `Baris ${rowNumber}: mapel "${subjectName || "-"}" tidak terhubung ke siswa ${student.full_name || "-"}.`
          );
        ...
        if (
          ...
          if (
            ...
            errors.push(
              `Baris ${rowNumber}: laporan sudah ${getStatusLabel(
                approvalStatus
              )} dan tidak bisa diubah dari import.`
            );
        ...
        const importedMidScore =
          excelScoreToNumber(
            row.UTS
          );

        const importedFinalExamScore =
          excelScoreToNumber(
            row.UAS
          );

        const importedProcessScore =
          excelScoreToNumber(
            row[
              "Proses KBM"
            ]
        ...
        const payload = {
          ...
          /*
           * Catatan Excel kosong
           * tidak menghapus
           * catatan lama.
           */
          teacher_comment:
            importedTeacherComment ||
            existingReport
              ?.teacher_comment ||
            null,
      ...
      setErrorMessage(
        error instanceof Error
          ? error.message
          : "Gagal membaca/import file Excel."
      );
    } finally {
      setImportingExcel(
        false
  ...
  function handleExportExcel() {
    if (
      ...
      alert(
        "Tidak ada data laporan akademik yang bisa diexport."
      );
      ...
      <section className="space-y-7">
        ...
        <div className="flex flex-col justify-between gap-4 xl:flex-row xl:items-end">
          ...
          <div>
            ...
            <p className="mt-2 max-w-[850px] text-[15px] leading-6 text-[#6F5549]">
              Laporan akademik bisa diisi langsung di web atau upload file raport/laporan akademik jika sekolah sudah memiliki format sendiri.
            </p>
          </div>
          ...
          <div className="flex flex-wrap gap-3">
            ...
            <button
              ...
              className="flex h-11 w-fit items-center gap-2 rounded-xl border border-[#DCC8B6] bg-white px-5 text-[14px] font-extrabold text-[#8C0F2D] shadow-sm transition hover:bg-[#FFF8EF] disabled:cursor-not-allowed disabled:opacity-60"
            >
        ...
        studentOptionsForForm.length ===
          ...
          <div className="rounded-2xl border border-[#E8D6C1] bg-white px-5 py-4 text-[14px] leading-6 text-[#6F5549]">
            Belum ada siswa yang terhubung ke guru ini. Hubungkan siswa dengan guru dan mapel dari menu Kepala Sekolah → Siswa.
          </div>
        ) : null}
        ...
        <div className="overflow-hidden rounded-[22px] border border-[#E1CFBE] bg-white shadow-sm">
          ...
          <div className="border-b border-[#EADACA] px-6 py-5">
            ...
            <p className="mt-1 text-[14px] text-[#6F5549]">
              Draft, pending, dan rejected bisa dikelola. Approved hanya bisa dilihat.
            </p>
          </div>
            ...
            <div className="sticky top-0 z-10 flex items-center justify-between border-b border-[#E1CFBE] bg-[#FFF8EF] px-6 py-5">
              ...
              <div>
                <h2 className="text-[22px] font-extrabold text-[#2B1B18]">
                  {form.id
                    ? "Edit / Tambah Nilai Akademik"
                    : "Input / Kelola Nilai Akademik"}
                </h2>

                <p className="mt-1 text-[14px] text-[#6F5549]">
                  Tambahkan UH, tugas, UTS/UAS, atau nilai berikutnya pada laporan siswa yang sama tanpa membuat laporan baru.
                </p>
              </div>
            ...
            <div className="space-y-5 px-6 py-6">
              ...
              <div className="rounded-2xl border border-[#E1CFBE] bg-white px-5 py-4">
                ...
                <p className="mt-2 text-[13px] leading-6 text-[#6F5549]">
                  Jika nilai ingin dikelola di web, isi kolom nilai di bawah. Jika sekolah sudah punya file raport/laporan, upload file saja juga bisa.
                </p>
              </div>
...
function ScoreInput({
  ...
  return (
    <label className="block">
      ...
      <p className="mb-2 text-[13px] font-bold text-[#6F5549]">
        {label}
      </p>
